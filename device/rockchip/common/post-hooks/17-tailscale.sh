...
if [ -z "$QEMU_BIN" ]; then
	error "qemu-*-static not found on host; cannot chroot to install tailscale."
	exit 1
fi
...
"${CHROOT_ENV[@]}" apt-get -y install curl ca-certificates gnupg
...
"${CHROOT_ENV[@]}" apt-get update
"${CHROOT_ENV[@]}" apt-get -y install tailscale
