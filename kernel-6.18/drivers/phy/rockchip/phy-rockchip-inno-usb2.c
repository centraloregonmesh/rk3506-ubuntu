...
static inline bool property_is_valid(const struct usb2phy_reg *reg)
{
	return reg->offset || reg->bitend || reg->bitstart ||
...
static int rockchip_usb2phy_clk480m_prepare(struct clk_hw *hw)
{
	...
	if (!property_is_valid(&rphy->phy_cfg->clkout_ctl))
		return 0;
...
static int rockchip_usb2phy_power_on(struct phy *phy)
{
	...
	/*
	 * The vendor 6.1 tree only resets OTG PHYs on RK3576/RK3588 here.
	 * Resetting the RK3506 PHY while binding the gadget can leave the UDC
	 * with no backing device, which surfaces as "failed to start rockchip:
	 * -ENODEV" from the gadget core.
	 */
	if (rport->port_id == USB2PHY_PORT_OTG &&
