...
static unsigned int rk3506_gmac_offset(struct rk_priv_data *bsp_priv)
{
	return bsp_priv->id == 1 ? RK3506_GRF_SOC_CON11 : RK3506_GRF_SOC_CON8;
}

static void rk3506_set_to_rmii(struct rk_priv_data *bsp_priv)
{
	regmap_write(bsp_priv->grf, rk3506_gmac_offset(bsp_priv),
		     RK3506_GMAC_RMII_MODE);
}
			    ...
			    phy_interface_t interface, int speed)
{
	...
	regmap_write(bsp_priv->grf, rk3506_gmac_offset(bsp_priv), val);
				       ...
				       bool input, bool enable)
{
	...
	regmap_write(bsp_priv->grf, rk3506_gmac_offset(bsp_priv), value);
}
